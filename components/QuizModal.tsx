...
const questions: Question[] = [
  {
    ...
    options: [
      { label: "Sore, tired muscles after workouts or long days", value: "Sore muscles", icon: <Activity size={24} /> },
      { label: "Dry, rough, or irritated skin that needs deep moisture", value: "Dry skin", icon: <Droplet size={24} /> },
  ...
  {
    id: 5,
    text: "What's your body care budget per product?",
    options: [
      { label: "Under €15 – looking for essentials", value: "Under €15" },
      { label: "€15-€30 – willing to invest in quality", value: "€15-€30" },
      { label: "€30-€50 – want premium formulas", value: "€30-€50" },
      { label: "€50+ – only the best for my body", value: "€50+" },
    ]
  }
];
...
const QuizModal: React.FC<QuizModalProps> = ({ isOpen, onClose }) => {
  ...
  const calculateResults = () => {
    ...
    // Logic Rules
    if (q1 === "Sore muscles") addRec('1', "You mentioned sore muscles after workouts. This rich, targeted balm is our strongest formula—designed specifically for overworked muscles.");
    ...
    if (q5 === "Under €15") {
      addRec('4', "Premium botanical care at an essential price point.");
      addRec('6', "Artisan quality that fits your budget.");
    }
    
    if (q5 === "€30-€50" || q5 === "€50+") {
      addRec('2', "Luxurious care worth the investment.");
      addRec('3', "A premium oil blend for the ultimate self-care experience.");
    }
    ...
    // Track completion
    console.log("Quiz Completed", finalRecs);
  };
            ...
            <div className="p-8 animate-fade-in-up">
              ...
              <div className="space-y-6 mb-10">
                {recommendations.map((rec, idx) => (
                  <div 
                    key={rec.product.id} 
                    className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm flex flex-col md:flex-row gap-6 items-center animate-fade-in-up"
                    style={{ animationDelay: `${idx * 150}ms` }}
                  >
                    <div className="w-full md:w-32 flex-shrink-0 aspect-square rounded-lg overflow-hidden bg-gray-100">
                      <img src={rec.product.image} alt={rec.product.name} className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-grow text-center md:text-left">
                      <div className="flex flex-col md:flex-row md:items-center justify-between mb-2">
                        <h3 className="font-playfair font-bold text-xl text-teal-900">{rec.product.name}</h3>
                        <span className="text-lg font-bold text-coral">€{rec.product.price}</span>
                      </div>
                      <p className="text-sm text-gray-500 italic mb-3">{rec.reason}</p>
                      <div className="flex items-center justify-center md:justify-start gap-2 mb-4">
                        <span className="text-xs font-semibold text-gray-400 uppercase">Intensity</span>
                        <div className="flex gap-1">
                          {[1, 2, 3].map((i) => (
                            <div 
                              key={i} 
                              className={`w-2 h-2 rounded-full border border-teal-800 ${i <= rec.product.intensity ? 'bg-teal-800' : 'bg-transparent'}`}
                            />
                          ))}
                        </div>
                      </div>
                      <a 
                        ...
                        onClick={onClose}
                        className="inline-block bg-coral hover:bg-orange-500 text-white font-bold py-2 px-6 rounded-full text-xs tracking-widest transition-colors"
                      >
                        VIEW THIS PRODUCT →
                      </a>
                    </div>
